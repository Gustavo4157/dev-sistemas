...
def mudar_status(
    ...
    if i, t in enumerate(banco):
        if t.id == tarefa_id:
            if t.status == StatusEnum.cancelada and dados.status != StatusEnum.cancelada:
                raise HTTPException(status_code=400, detail='Tarefas canceladas,'
                'não podem ser alteradas')
                atual = t.model_dump()
                atual['status'] = dados.status
                banco[i] = TarefaSaida(**atual)
                return banco[i]
            return HTTPException(status_code=404,detail='Tarefa nao encontrada')
