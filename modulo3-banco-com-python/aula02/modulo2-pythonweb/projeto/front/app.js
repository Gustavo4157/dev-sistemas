...
// <---------------------->
                ...
                ${t.tags && t.tags.length ? t.tags.map(tag => `<span class="badge badge-tag">#${tag}</span>`).join('') : ''}
            </div>
// <---------------------->
            <div class="card-acoes">
                ${t.status !== 'concluida' && t.status !== 'cancelada' ? `<button class="btn-concluir" data-id="${t.id}">Concluir</button>` : ''}
...
// <---------------------->
    ...
    const responsavel = document.querySelector('#filtro-responsavel').value;
...
// <---------------------->
    if (e.target.classList.contains('btn-concluir')) {
        await fetch(`${BASE}/tarefas/${id}/status`, {
